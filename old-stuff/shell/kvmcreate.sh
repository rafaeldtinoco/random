...
sudo debootstrap --include=less,vim,sudo,openssh-server,bash-completion,wget,rsync,git,build-essential,gdb,crash $release /target http://us.archive.ubuntu.com/ubuntu/
...
# deb-src http://us.archive.ubuntu.com/ubuntu/ RELEASE-proposed main restricted universe multiverse
...
sudo chroot /target /bin/bash -c "apt-get install -y linux-image-generic linux-headers-generic"
